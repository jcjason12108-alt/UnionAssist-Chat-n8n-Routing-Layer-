...
if (file_exists(__DIR__ . '/plugin-update-checker/plugin-update-checker.php')) {
    ...
    add_filter(
        $n8n_union_chat_update_checker->getUniqueName('vcs_update_detection_strategies'),
        static function (array $strategies): array {
            return isset($strategies['branch']) ? ['branch' => $strategies['branch']] : $strategies;
        }
    );

    if (defined('N8N_UNION_CHAT_UPDATE_GITHUB_TOKEN')) {
        $n8n_union_chat_github_token = N8N_UNION_CHAT_UPDATE_GITHUB_TOKEN;
    } elseif (defined('PLUGIN_UPDATE_GITHUB_TOKEN')) {
        $n8n_union_chat_github_token = PLUGIN_UPDATE_GITHUB_TOKEN;
    } else {
        $n8n_union_chat_github_token = getenv('N8N_UNION_CHAT_UPDATE_GITHUB_TOKEN') ?: getenv('PLUGIN_UPDATE_GITHUB_TOKEN');
    }
